...
namespace Confuser.Renamer.BAML
{
    internal class BAMLAnalyzer
    {
        ...
        private readonly Dictionary<ushort, Tuple<ISDnlibDef, AttributeInfoRecord, TypeDef>> attrRefs = new Dictionary<ushort, Tuple<ISDnlibDef, AttributeInfoRecord, TypeDef>>();
        ...
        public Tuple<ISDnlibDef, AttributeInfoRecord, TypeDef> ResolveAttribute(ushort attrId)
        {
            if ((short)attrId < 0)
            {
                Tuple<KnownTypes, PropertyDef, TypeDef> info = things.Properties((KnownProperties)(-(short)attrId));
                return Tuple.Create<ISDnlibDef, AttributeInfoRecord, TypeDef>(info.Item2, null, info.Item3);
            }
            return attrRefs[attrId];
        }

        private void AddTypeSigReference(TypeSig typeSig, INameReference<ISDnlibDef> reference)
        {

...
namespace dnlib.NetCore.DotNet.Writer
{
    ...
    {
        private const uint HOT_HEAP_MAGIC = 0x484F4E44; // "HOND"
        private const int MAX_TABLES = (int)Table.GenericParamConstraint + 1;
        private const uint HOT_HEAP_DIR_SIZE = 4 + MAX_TABLES * 4;
        private const uint HH_ALIGNMENT = 4;

        private uint totalLength;
        private readonly HotTable[] headers = new HotTable[MAX_TABLES];
        private readonly List<HotPool> hotPools = new List<HotPool>();
        ...
        private static void WriteAlign(BinaryWriter writer, ref uint offs)
        {
